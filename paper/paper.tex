...
\begin{definition}
  ...
  The payoff that $s_1$ gets when playing against $s_2$ is
  \begin{equation*}
    v_{s_1}(s_2) = \sum_{(c_1,c_2)} \pi_{c_1,c_2} \cdot r(G_{s_1}(c_1),G_{s_2}(c_2))
  \end{equation*}
  where $G_s(c)$ is the output of strategy $s$ at state $c$. The sum is taken over all states $(c_1,c_2)$ in the $s_1$-$s_2$ Markov chain.
\end{definition}

For notational convenience, we may also make $r(G_{s_1}(c_1),G_{s_2}(c_2))$ into a vector, denoted by $r$, and write this as the dot product \begin{equation*}
...
We can now state our results! Together, the following two conjectures imply that in the setup described here, mutual cooperation arises as the only stable choice.
...
\begin{conjecture}
  \label{pavlovtheorem}
  Suppose $2R > T + P$. Then, the Pavlov strategy, depicted in \cref{pavlovfigure}, is evolutionarily stable.
\end{conjecture}
...
\subsection{Evolutionary Stability Implies Utilitarianism}
  ...
  The following proof is relatively involved, and we believe that a simpler proof exists, but haven't been able to find one.
    ...
    \begin{proof}[Proof of \cref{evolutionarystable1}]
      ...
      The underlying idea is to create $s_2$ such that $s_1$ can see no difference between itself and $s_2$, while $s_2$, on the other hand, can. In that case, we would have that $v_{s_1}(s_2) = v_{s_2}(s_1) = v_{s_1}(s_1)$, and could construct $s_2$ such that it always cooperates when it recognizes itself, thereby yielding $v_{s_2}(s_2) = R$. This would give $s_2$ a higher fitness than $s_1$. The rest of this proof executes this plan in detail.

      We create the strategy $s_2$ as follows. First, we copy all of $s_1$ into $s_2$. Let $\mathcal{C}_1$ be the set of
       absorbing strongly connected components of $s_1$. The key idea, now, is to replace each original absorbing strongly connected component $C_1 \in \mathcal{C}_1$ with a new absorbing component $C_2$, which has the capability of self identifying.

      We now construct $C_2$, given $C_1$. \Cref{c2flowchart} shows the high-level construction of $C_2$. All of the steps succeed with probability $1 - O(p)$. The first step makes sure that $C_2$ does not deviate from $s_1$ at all, until $s_1$ has reached an absorbing strongly connected component. Once $s_1$ is in an absorbing strongly connected component, we already know that the time average distribution does not depend on the particular state in $s_1$, which means that even if $s_1$ now ``figures out'' that it is not playing against itself anymore, there is nothing it can do about it. The next step makes sure that, if $s_2$ is playing against itself, it is in sync; i.e., with probability $1 - O(p)$ they will both copies of $s_2$ will be in the same state after this step. The third step now figures out what action $s_1$ would take at some large finite time $T$, and then outputs the exact opposite action at time $T$. At that point, if its opponent outputs $s_1$'s expected action, it transitions into the absorbing strongly connected component $C_1$, but if its opponent outputs the opposite of that action, it has successfully recognized itself and enters an always cooperating state.

      \begin{figure}
        \centering
        \begin{tikzpicture}[node distance=0.9cm,initial text=]
          \node[elliptic state, initial, initial where=above] (c_0) {1. Wait until $s_1$ is in an absorbing SCC, while simulating $C_1$.};
          ...
          \node[state] (c_7) [below=of c_4] {$C_1$};
        ...
        \end{tikzpicture}
        \caption{High level construction of $C_2$, the absorbing component replacing $C_1$ when creating $s_2$ in the proof of \cref{evolutionarystable1}. Note that $C_2$ contains exactly 2 absorbing strongly connected components.}
        \label{c2flowchart}
      \end{figure}
      
      This concludes the construction of $s_2$. We now want to prove that this $s_2$ can in fact invade $s_1$. To do that, we will first assume that the below 3 claims are correct, i.e., that the construction is possible and works as specified, and finish the proof of our theorem using those assumptions. Then, we will prove the 3 claims.

      \begin{claim}
        \label{claimcanwaitfors1}
        We can construct a non-absorbing component with indistinguishable output from the component $C_1$, such that when we leave the component, $s_1$ will be in an absorbing strongly connected component with probability $1 - O(p)$.
      \end{claim}
      ...
      \begin{subproof}
        ...
        That $v_{s_1}(s_1) = \gamma + O(p)$ follows by our definition of $\gamma$.

        Consider first the case of $s_1$ playing against $s_2$. By \cref{claimcanwaitfors1}, with probability $1 - O(p)$, $s_2$ is indistinguishable from $s_1$ until $s_1$ reaches an absorbing strongly connected component $C_1'$. This means that the probability for reaching the absorbing component $(C_1', C_2)$ in the $s_1$-$s_2$ Markov chain, is up to a factor of $1 - O(p)$ the same as if we hadn't replaced $C_1$ by $C_2$, which is just the probability of reaching the absorbing component $(C_1', C_1)$ in the $s_1$-$s_1$ Markov chain. Thus, 
        \begin{multline*}
