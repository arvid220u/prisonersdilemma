...
\begin{lemma}
  ...
  \begin{equation*}
    v_{s_1}(s_2) = \sum_{(c_1,c_2) \in S_{s_1,s_2}} \pi_{c_1,c_2}^{(c_{\text{start}}(s_1), c_{\text{start}}(s_2))} \cdot r(G_{s_1}(c_1),G_{s_2}(c_2)).
  \end{equation*}
\end{lemma}

We may also make $r(G_{s_1}(c_1),G_{s_2}(c_2))$ into a vector, denoted by $r$, and write this as \begin{equation*}
  v_{s_1}(s_2) = \pi^{(c_{\text{start}}(s_1), c_{\text{start}}(s_2))} \cdot r.
\end{equation*}
...
\begin{figure}
  ...
  \caption{Pavlov.}
\end{figure}
  \todo{use tikz for these figures!}
...
With these lemmas, we are now ready to prove our first theorem.
    ...
    \begin{proof}[Proof of \cref{evolutionarystable1}]
      ...
      and the outputs as \begin{align*}
        ...
        G(c_1) &= C.
      \end{align*}
      Let the start state of $s_2$ be $c_0$. \todo{add a figure!! the construction is simple but this description is deceivingly hard}
